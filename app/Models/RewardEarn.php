...
class RewardEarn extends Model
{
    use HasFactory;

    protected $table = 'reward_earn';
    ...
    public function scopeAvailableForRedemption($query, $user)
    {
        return $query->where('user_id', $user->id)->where('AgencyID', $user->AgencyID)
            ->where('rewardearn', '>', 0)
            ->orderBy('created_at', 'asc'); // FIFO - First In First Out
    }

    /**
     * Get the loyalty card associated with the reward earning.
     */
    ...
    /**
     * Get the reward associated with the reward earning.
     */
    public function reward()
    {
        return $this->belongsTo(LoyaltyReward::class, 'reward_id', 'reward_id');
    }
    ...
    {
        $result = RewardEarn::select(
            DB::raw('CAST(SUM(reward_earn.rewardearn) AS DECIMAL(10,2)) as total_rewardearn'),
            DB::raw('CAST(SUM(reward_earn.order_amount) AS DECIMAL(10,2)) as total_order_amount'),
            'users.id',
            'users.name',
        )
            ->leftJoin('users', 'users.id', '=', 'reward_earn.user_id')
            ->where(function ($query) use ($User) {
                if ($User->UserType == 1) {
                    ...
                    $query->where('reward_earn.AgencyID', $User->AgencyID);
                }
            })
            ->where(function ($query) use ($post) {
                if (isset($post['user_id']) && $post['user_id'] > 0) {
                ...
                if (isset($post['dateFrom']) && isset($post['dateTo']) && !empty($post['dateFrom']) && !empty($post['dateTo'])) {
                    $query->whereDate('reward_earn.created_at', '>=', $post['dateFrom'])
                        ->whereDate('reward_earn.created_at', '<=', $post['dateTo']);
                }
            })->first();
        return $result;
    }
